...
def check_file(filepath):
    ...
    for key, val in claimed.items():
        ...
        # Qualified leads or qualified appointments (check BEFORE general leads)
        elif ('qualified' in kl and ('lead' in kl or 'appointment' in kl)) and 'cost' not in kl and 'avg' not in kl:
            actual_count = total_qualified
            if actual_count > 0 and abs(parsed - actual_count) > 1:
                errors.append(f"  MISMATCH [{key}]: Claimed {val} but monthly sum = {int(actual_count)}")
        # Trial session bookings / appointments (these should map to deals — but NOT "qualified appointments" which maps to qualified)
        elif ('trial session' in kl or 'session booking' in kl or 'appointment' in kl) and 'cost' not in kl and 'avg' not in kl and 'qualified' not in kl and 'total appointment' not in kl:
            actual_count = total_deals
            if actual_count > 0 and abs(parsed - actual_count) > 1:
                errors.append(f"  MISMATCH [{key}]: Claimed {val} but monthly sum = {int(actual_count)}")
        # Sessions/Leads/Total Appointments (general)
        elif ('session' in kl or ('total' in kl and 'lead' in kl) or ('total' in kl and 'appointment' in kl)) and 'cost' not in kl and 'avg' not in kl and 'qualified' not in kl:
